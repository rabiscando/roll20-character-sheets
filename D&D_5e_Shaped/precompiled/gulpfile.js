	...
	for (var i = start; i < limit; i++) {
		...
		if(i === 0) {
			template = template
				...
				.replace(/\x7B\x7Blevel_num_readable_pt_br\x7D\x7D/g, 'Truque')
			...
			template = template
				...
				.replace(/\x7B\x7Blevel_num_readable_pt_br\x7D\x7D/g, 'Nível ' + i.toString())
...
function skills (file) {
	...
	skillsJson.forEach(function(skill) {
		...
		skills.push(ggTemplate
				...
				.replace(/\x7B\x7Bfriendly_name_pt_br\x7D\x7D/g, skill.name_pt_br)
				...
				.replace(/\x7B\x7Bdescription_pt_br\x7D\x7D/g, skill.description_pt_br)
		);
	});
	return skills.join('\n\n');
}
